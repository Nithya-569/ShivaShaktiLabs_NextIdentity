...
export default function SignUpPage() {
  ...
  async function handleSubmit() {
    ...
    // 2. Save optional profile fields
    if (data?.user) {
      ...
      if (upsertErr) {
        console.error("Profile update error:", upsertErr);
      }
    }

    setLoading(false);

    // 3. Handle Confirmation/Redirect
      ...
      <div className="min-h-screen bg-[#F4F7F9] flex items-center justify-center px-4">
        <div className="w-full max-w-md bg-white/80 backdrop-blur-xl border border-white/60 rounded-3xl shadow-[0_8px_40px_rgba(0,0,0,0.08)] p-10 text-center">
          ...
          <p className="text-xs text-slate-400">
            Already verified?{" "}
            <Link href="/auth/signin" className="text-teal-600 font-bold hover:underline transition-colors">Sign in</Link>
          </p>
        </div>
      </div>
    );
          ...
          {step === 2 && (
            <div className="flex flex-col gap-4">
              ...
              <div className="flex gap-3 mt-1">
                <button onClick={() => { setStep(1); setError(""); }} className="flex-1 py-3.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-2xl font-bold text-sm transition-colors transition-shadow">
                  ← Back
                </button>
                <button
